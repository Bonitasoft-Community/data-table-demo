...
= API REST de Request
API REST qui gère NomdeRessource

The project **requestRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/requestRestAPI.git
cd requestRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `requestRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `requestRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
